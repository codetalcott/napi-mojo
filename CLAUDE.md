...
**A prebuilt platform package is not source, and its `license` must say so.** `@napi-mojo/<platform>` ships third-party runtime libraries beside `index.node`; 0.13.0 declared `"MIT"` while carrying the Mojo runtime and, on Linux, GCC's `libstdc++.so.6` and `libgcc_s.so.1`. The declaration is now per-platform in `scripts/platforms.mjs` (`license` + `licenseFiles`), the texts live in `licenses/`, `publish.yml` stages them from that one declaration, and `check-platforms.mjs` fails if a manifest and its texts disagree. macOS carries no GCC runtime — Mojo links the system libc++ there — so it does not ship GPLv3; shipping it anyway would be misleading, not thorough. `bundle-runtime.sh` rewrites every bundled library's rpath/install name, so the Apache §4(b) modification notice applies and `licenses/NOTICE.bundle.txt` records it. **The bundled `libstdc++.so.6` is 23.9 MB of a 27 MB package and is probably redundant** — the runtime needs at most `GLIBCXX_3.4.30`, its un-bundlable `GLIBC_2.35` floor already implies a host that provides it, and deleting both GCC libraries from the published tarball leaves it fully working. **Both guards that the removal was waiting on now exist; the removal itself is still deliberately held.** `scripts/check-glibc-floor.mjs` reads ELF `.gnu.version_r` and holds the shipped set to the WORST host its own glibc floor admits (a `GLIBC_2.34` build is held to RHEL 9's `GLIBCXX_3.4.29`, not Ubuntu 22.04's `3.4.30`) — pre-bundle in `test.yml` every PR, and on the staged set in `publish.yml`; an **unresolved dependency fails it**, because the library it cannot open is the one carrying the requirement, and an empty tally otherwise reads as a clean bill. `consume-oldest-linux` in `publish.yml` (which `publish` needs) runs three docker cases per Linux platform with no checkout and no toolchain: the bundle loads on Debian 12, the same bundle **with the GCC runtime deleted** also loads, and it is refused below the floor with a loader version error rather than a crash — so the case for removal is re-measured every release. See [`docs/plan-distribution.md`](docs/plan-distribution.md).
