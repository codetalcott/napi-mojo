...
- Long-term refactor (not blocking): change wrapper signatures for genuinely null-sentinel inputs (`null_code`, `null_resource`, etc.) to take `Optional[NapiValue]` so the intent is type-encoded. Tracked but deferred — `(unsafe_from_address=0)` is sufficient until a future nightly removes it too.
