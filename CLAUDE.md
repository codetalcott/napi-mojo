...
- **Dynamically created inner callbacks**: their data slot either IS the bindings pointer (`inner_callback_fn`) or a capture struct that embeds it (`createAdder`'s `TypedPayload.bindings_addr`), freed by the collector through `create_with_data`'s `finalize_cb` overload.
...
src/napi/framework/js_object.mojo        # JsObject.create(), set_property(), set_named_property(), set(), get(), get_property(), get_named_property(), has(), has_property(), keys(), has_own(), delete_prop(), instance_of(), freeze(), seal(), add_finalizer(), prototype()
...
src/napi/framework/js_promise.mojo       # JsPromise.create(), resolve(), reject(), on_settled(); Settlement.read(), user_data()
...
src/addon/runtime_ops.mojo               # asyncRuntimeInitOk() + parallelSquares() — make parallelize_safe's two silent failure modes testable
...
spike/await_probe.mojo (+ .cjs)          # throwaway: proves no blocking wait on a JS Promise works inside a callback
...
156 exported functions covering the full N-API surface. See `docs/EXPORTS.md` for the complete table.
...
- **`parallelSquares(n, scale)`** covers the OTHER silent mode: the work runs but the captures are wrong. It returns a `Float64Array` where `[i] = i * i * scale` computed through `parallelize_safe`, so a broken capture or a skipped index cannot produce a correct array (every element depends on both captured values, and the buffer is pre-filled with a sentinel). It is also the only thing that **instantiates `parallelize_safe` in the addon graph** — before it existed, that function's body was never elaborated by `build.sh`, so it could fail to compile with the build and the entire suite green, which is exactly what the Mojo 1.1.0 bump hit. Mutation-checked: reverting `runtime.mojo` to bare `capturing` makes it SIGSEGV (node exits 139), so the regression surfaces as a crashed Jest worker rather than a clean assertion diff.
...
1. **Mojo code cannot wait for a JS Promise — and not because Mojo lacks
   `await`.** It has one (`async def`, `Task`, AsyncRT), but that suspends
   Mojo coroutines. Host-mode code runs synchronously on the JS thread, a
   promise settles only after that code returns to the event loop, and N-API
   has no call that reads a promise's state or result. Measured from inside a
   callback: polling for 500 ms, closing a callback scope and re-entering
   `uv_run` all failed to observe a settlement; `uv_run` even ran a timer's JS
   callback on the nested stack and *still* did not run the promise's `.then`.
   So: synchronous APIs (`readFileSync`), or a continuation via
   **`JsPromise.on_settled`**, which `mojo_main` returns past and the event
   loop fires later. **Do not add a blocking await helper** — that experiment
   is why. Write continuations on `on_settled`, never a hand-rolled `.then()`:
   the first hand-rolled version (in `src/addon/host_ops.mojo`) crashed the
   process on rejection, swallowed its own errors, and leaked its payload and
   a strongly-referenced `onResult` for every promise that rejected or never
   settled (209.7 MB for 200 closures over 1 MB). `on_settled` attaches both
   handlers, passes JS captures as bound arguments the GC traces (no
   `napi_ref`), adopts native `data` into a GC finalizer (nothing is freed
   when the continuation fires — it may never fire), and returns the promise
   `.then()` made so a failure is observable. `tests/finalizer_gc.test.js`
   pins the lifetimes; each of those three regressions was mutation-checked
   against the suite. An await that suspends *Mojo* code needs a worker-thread
   bridge in the napi-rs shape — see
   [`docs/plan-promise-bridge.md`](docs/plan-promise-bridge.md), deliberately
   not built.
...
> **This is one instance of a broader class — artifacts verified only when something happens to reach them, which rot silently while every signal reads green.** Method bodies, struct definitions, generator templates, docstrings, the export table, the binary-format parsers and the doc anchors are all in it, and prose describing a mechanism still has no gate at all (Mojo warnings and the doc code blocks got theirs on 2026-09-17, above). The class, the evidence, the inventory of what is and is not gated, and the decisions — including what stays ungated on purpose and the named trigger for revisiting each — are in [`docs/plan-lazily-checked-artifacts.md`](docs/plan-lazily-checked-artifacts.md). **Read it before proposing a new gate** — some rows there are correctly ungated, and it records the constraints a gate has to meet here (mechanical not judgment; must fail on the known bug; ratchet not mute; never a new *required* check).
...
**Module exports carry `writable | enumerable | configurable`, not `napi_default`.** `ModuleBuilder.method` and `module.mojo`'s `register_method` are the two sites. With attributes 0 an export was non-enumerable, non-writable AND non-configurable: `Object.keys(addon)` returned the five classes (registered by another path, and enumerable), `{...addon}` lost every function, and assigning over an export silently did nothing — two behaviours in one module. **Class prototype members deliberately keep 0**, because a JS class method is non-enumerable and matching that is correct; `js_class.mojo` and `ClassBuilder`'s member sites must not be swept along with the module ones. Note the side effect: `tests/typescript.test.js`'s "every enumerable function is declared in the .d.ts" loop was **vacuous** while only classes were enumerable, and is now live over every export.

**`napi-mojo release --scaffold` writes an addon author's publishing setup** — `optionalDependencies` on one prebuilt package per platform, a loader that prefers a local build over the registry, `npm/<platform>/` manifests, and a release workflow whose consume job has **no checkout and no toolchain** (add one "to get a test app" and it starts proving something else). It patches `package.json` and each `npm/<platform>/package.json` (merging `optionalDependencies`, never adding `files` to a package that had none — that would drop `main` from the tarball), writes `index.js` and `release.yml` only when absent unless `--force`, gives platform manifests a licence expression and texts covering what they carry plus the root `repository` (provenance rejects a package without one), and reads the platform list from `scripts/platforms.mjs` rather than keeping a second copy. **Its e2e runs the CLI from the packed tarball, not the checkout**: `bin/napi-mojo.mjs` imports `scripts/platforms.mjs` and the generated workflow calls `scripts/check-portable.mjs`, so both must be in `package.json` `files` — and running from the repo cannot see a missing entry, because the file is on disk either way, while a missing one breaks *every* CLI invocation for an installed user. Any new runtime dependency of the CLI or its templates gets a `files` entry in the same commit — `licenses/` is one: the scaffold copies the Mojo runtime licence out of the INSTALLED package. **The first publish goes through `release --bootstrap`**, a `0.0.0-bootstrap.0` placeholder on the `bootstrap` dist-tag, never `npm publish npm/<platform>`: those directories hold only a manifest until CI stages the binary, and a version published empty can never be republished.

**A prebuilt platform package is not source, and its `license` must say so.** `@napi-mojo/<platform>` ships the Mojo runtime beside `index.node`; 0.13.0 declared `"MIT"` while carrying it and, on Linux, GCC's `libstdc++.so.6` and `libgcc_s.so.1`. The declaration is per-platform in `scripts/platforms.mjs` (`license` + `licenseFiles`), the texts live in `licenses/`, `publish.yml` stages them from that one declaration, and `check-platforms.mjs` fails if a manifest and its texts disagree. All three platforms now read `MIT AND Apache-2.0 WITH LLVM-exception`; the shape stays per-platform because it did NOT agree while Linux bundled GCC's runtime, and that is what kept the declaration honest. `bundle-runtime.sh` rewrites every bundled library's rpath/install name, so the Apache §4(b) modification notice applies and `licenses/NOTICE.bundle.txt` records it.

**The GCC runtime is NOT bundled — `HOST_PROVIDED` in `bundle-runtime.sh`.** `libstdc++.so*` and `libgcc_s.so*` are in the closure and are skipped, taking linux-x64 from 26.1 MB to 2.4 MB unpacked (9.4 → 0.9 MB tarball, 91%) and removing the `GPL-3.0-or-later WITH GCC-exception-3.1` half of the Linux declaration. They are redundant because the Mojo runtime needs at most `GLIBCXX_3.4.30` while its **un-bundlable** `GLIBC_2.35` floor already implies a host that provides it — glibc is the loader, so that floor cannot be shipped around — and Node itself links both libraries. **Bun and Deno do not** (Bun's glibc build carries its C++ runtime statically; Deno links only libgcc_s), so under them the IMAGE must provide libstdc++: `oven/bun:*-distroless` and `denoland/deno:alpine` do not and cannot load the bundle — a measured trade recorded in `docs/plan-distribution.md`, not a bug. **Do not re-add them because something failed to load; find out which floor moved.** `scripts/check-glibc-floor.mjs` (in `test.yml` every PR, pre-bundle) fails if the runtime's `GLIBCXX`, `CXXABI` or libgcc_s `GCC_` requirement rises above what its own glibc floor guarantees on the WORST host that floor admits (a `GLIBC_2.34` build is held to RHEL 9's `3.4.29`, not Ubuntu 22.04's `3.4.30`); an **unresolved dependency fails it**, because the library it cannot open is the one carrying the requirement and an empty tally otherwise reads as a clean bill. `consume-oldest-linux` in `publish.yml` (which `publish` needs) runs docker cases per Linux platform with no checkout and no toolchain: the bundle loads on Debian 12, and on `oven/bun:*-slim` and `denoland/deno:distroless` (pinned to `test.yml`'s runtime versions); it is refused on `oven/bun:*-distroless` with the loader's `libstdc++.so.6` error rather than a crash; the bundle contains **no** GCC runtime (guarding the removal against a closure change putting it back), and it is refused below the floor with a loader version error rather than a crash. The residual risk is a host with glibc ≥ 2.35 and an older libstdc++ — constructible, unrepresented in any table or container, and it fails at `require()` with a version error rather than corrupting. See [`docs/plan-distribution.md`](docs/plan-distribution.md).

**Artifact portability gate** (`publish.yml`, before the upload step): `scripts/check-portable.mjs` reads the bundled artifact's Mach-O / ELF load commands and asserts that every recorded search path is self-relative and every dependency is a system library or shipped beside it. The "Verify bundled binary is self-contained" step above it clears `DYLD_*`/`LD_LIBRARY_PATH` and *loads* the binary — on the one machine where a stale absolute rpath still resolves, so it proves the bundle works there and nothing about a consumer. It runs against `/tmp/selfcontained/`, the consumer layout, not `build/`, and covers every library in `build/bundled-libs.txt` as well as `index.node`. **The verdict must model where the loader LOOKS, not where files happen to be** — every shortcut there shipped as a false pass: a neighbour counts only through a self-relative search path (no fallback to "beside the artifact"), a `@loader_path/` dependency must exist, `@executable_path` is node's directory, and a manifest entry that is absent fails. Its `--self-test` cases put the library IN the fixture directory, because an empty fixture hides exactly this class. **Parse the bytes, never shell out to `otool`/`readelf`** — that is ported lore, not preference: `otool` is macOS-only so a Linux job cannot read a macOS artifact at all, and `llvm-objdump` exists on macOS but prints ELF entries in another format, so the regexes matched nothing and a Linux artifact was reported *portable*. Three verdicts (broken / satisfiable / self-contained), because `self-contained` depends on the runtime-redistribution licensing question and collapsing them makes the gate either toothless or a release blocker. `--self-test` synthesises both formats and runs in `test.yml` on both platforms; it covers the distinction that is invisible when you only ever look at one file — an executable has no `LC_ID_DYLIB`, a dylib does, and treating the first dependency as the install name is the bug the parser was extracted for. Bundling itself is still verified only at release time.

**Cross-runtime gate** (`runtimes` job, non-required, ubuntu-latest): `scripts/check-runtimes.mjs` runs each scenario as its own **child process** under Node, Deno and Bun and compares the other two against Node. Child processes, not in-process assertions, because every defect it has found was a process-level event — Bun aborting, Deno printing a glibc double free at teardown *after* the last user line with a zero exit status — which an in-process comparison of return values cannot see. Runtime versions are pinned in `test.yml`: a red job must mean a regression here, not that Bun shipped this morning. `KNOWN_DEFECTS` in the script is a **ratchet, not a mute** — each entry must still reproduce *with its recorded signature* (any other failure is its own finding, so a timeout or a different crash cannot keep an entry alive), so a runtime that fixes its bug turns the gate red and the allowance cannot outlive the bug (same shape as `KNOWN_UNDOCUMENTABLE` in `check-docstring-coverage.mjs`). Non-required because a third-party runtime's regression must not block a merge.

**A load failure is explained by the loader, not the docs.** `load-error.js` (repo root, shipped, and copied verbatim into scaffolded projects — so it must stay dependency-free) maps dlopen messages to an error `code`, a fix and a `docs/TROUBLESHOOTING.md` anchor; `demo.js` and the scaffold's `index.js` call it, and must keep "not installed" (`require.resolve` fails) separate from "installed but cannot load" — `demo.js` once reported a missing libstdc++ as "No prebuilt demo binary". `consume-oldest-linux` loads through that loader and builds the doc's Bun-distroless recipe **extracted from the doc by heading**, so renaming `### Fix: copy the C++ runtime in` breaks the release job on purpose; the doc's Bun `RUN` check line is extracted the same way. **Bun auto-installs a missing bare specifier from npm when no `node_modules` directory exists** — `require.resolve('@napi-mojo/<platform>')` in an empty container returned the published 0.13.0 package (which bundles libstdc++), so Bun steps that went through `demo.js` tested npm's binary, not the bundle, and "proved" the Bun-distroless fix without exercising it. Container test apps get a `node_modules` directory and `check.cjs` refuses to run if a platform package resolves. **`bun -e "require(x)"` exits 0 when `x` throws or is missing** (Bun 1.3.7 and 1.3.11) — any Bun check or probe must use `bun -e "import 'x'"` or run a file, and a step whose success path is a warning must prove the check ran (the first version of this job took its warning path on every run because the check could not fail). `tests/load_error.test.js` holds the anchors, the support-matrix rows marked *checked on every release*, and the Bun/Deno pins (doc, `publish.yml` env, `test.yml`) together.

**Two upstream N-API bugs are on file, both reproducible from a plain C addon** (so neither is ours): Deno double-frees a `napi_async_cleanup_hook_handle` at env teardown — it affects any addon that leaves an async cleanup hook registered at exit, which is the normal use — and Bun aborts when two async cleanup hooks share a `(function, data)` pair, which N-API nowhere forbids. `addAsyncCleanupHook` therefore returns its handle (as an External) and `removeAsyncCleanupHook(handle)` removes that exact hook, rather than the old shape which registered a *second* hook with an identical pair just to have something to remove — and so never removed what add had registered. **The External owns a type-tagged `AsyncHookSlot`, never the raw handle**: `napi_remove_async_cleanup_hook` *frees* the handle, so an External carrying it directly let pure JS crash the process twice over — a second remove (use-after-free) and `removeAsyncCleanupHook(createExternal(1, 2))` (napi deleting an arbitrary pointer). Remove verifies the tag, then zeroes the slot *before* the call. Node, Bun and Deno all support type tags on Externals. **Never gate on `napi_get_version`**: Bun answers 9 and implements the v10 primitive-`napi_ref` behaviour this framework relies on. Test the behaviour. Measurements and reproducers: [`docs/plan-distribution.md`](docs/plan-distribution.md).
