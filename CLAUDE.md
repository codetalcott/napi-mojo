...
> **This is one instance of a broader class — artifacts verified only when something happens to reach them, which rot silently while every signal reads green.** Method bodies, struct definitions, generator templates, docstrings, the export table, the binary-format parsers and the doc anchors are all in it, and several things still have no gate at all (Mojo warnings, prose describing a mechanism, the 16 Mojo code blocks in the docs). The class, the evidence, the inventory of what is and is not gated, and the open design questions are in [`docs/plan-lazily-checked-artifacts.md`](docs/plan-lazily-checked-artifacts.md). **Read it before proposing a new gate** — some rows there are correctly ungated, and it records the constraints a gate has to meet here (mechanical not judgment; must fail on the known bug; ratchet not mute; never a new *required* check).
