...
def generic_double_array_fn(env: NapiEnv, info: NapiValue) -> NapiValue:
    try:
        ...
        for item in items:
            doubled.append(JsF64(item.val * 2.0))
...
def object_from_arrays_fn(env: NapiEnv, info: NapiValue) -> NapiValue:
    try:
        ...
        for i in range(len(keys)):
            ...
            obj.set(
                ba.b, env, key_js, JsNumber.create(ba.b, env, values[i]).value
            )
        return obj.value
    except:
        throw_js_type_error(
...
def object_to_arrays_fn(env: NapiEnv, info: NapiValue) -> NapiValue:
    try:
        ...
        for i in range(n):
            ...
            var v = JsNumber.from_napi_value(
                ba.b, env, JsObject(ba.arg0).get(ba.b, env, key_val)
            )
            keys.append(k)
            values.append(v)
        ...
        result.set_property(
            ba.b, env, "values", to_js_array_f64(ba.b, env, values)
        )
        return result.value
    except:
